...
async function handleTokenBoughts(rows: any[]) {
    for (const row of rows) {
        await tokenManager?.updateClientCoinData(row, "buy");
        await tokenManager?.updateClientTrades(row, "buy");
        console.log(
            JSON.stringify({
                event: "TokenBought",
                token: row.token,
...
async function handleTokenSolds(rows: any[]) {
    for (const row of rows) {
        await tokenManager?.updateClientCoinData(row, "sell");
        await tokenManager?.updateClientTrades(row, "sell");
        console.log(
            JSON.stringify({
                event: "TokenSold",
                token: row.token,
...
async function handleTrades(buyRows: any[], sellRows: any[]) {
    ...
    for (const trade of merged) {
        await tokenManager?.updateClientCoinData(trade.row, trade.event);
        await tokenManager?.updateClientTrades(trade.row, trade.event);
        console.log(
            JSON.stringify({
