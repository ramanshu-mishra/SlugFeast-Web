...
export const TradeOptions = memo(({ coin }: { coin: Coin }) => {
  ...
  async function _buyTokens() {
    if (!trade.active || buyLoading || sellLoading || processing || !address)
      return;
  ...
  return (
    <div className="select-none flex  h-fit flex-col gap-2 rounded-xl border border-neutral-800 bg-neutral-900 p-4">
            ...
            {(["Buy", "Sell"] as const).map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={` font-bold relative z-20 transition-colors duration-200 ${isActive ? "text-neutral-950" : "text-neutral-300"}`}
                  >
                    {tab}
                  </span>
                </button>
      ...
      <div className="flex items-center gap-2 px-2">
        {option == "Buy" && (
          <motion.span
            whileHover={{ backgroundColor: "--var(neutral-900)" }}
            className="text-sm px-2 rounded-md border border-neutral-700 bg-neutral-800 cursor-pointer"
            onClick={() => {
              setPov((e) => (e == "Eth" ? symbol : "Eth"));
            }}
          >
            switch to {pov == "Eth" ? symbol : "Eth"}
          </motion.span>
        )}
        <motion.span
          whileHover={{ backgroundColor: "--var(neutral-900)" }}
          className="text-sm px-2 rounded-md border border-neutral-700 bg-neutral-800 cursor-pointer ml-auto"
        >
          Slippage
        </motion.span>
      </div>
      <div className="flex justify-between px-2">
        ...
        {option == "Buy" && (
          <span>
            {pov == "Eth"
              ?  isConnected ? `${(Number(ethBalance?.value ?? 0) / Number(10 ** 18)).toFixed(5)} ETH`
              : `0 ETH` : balance
                ? balance.data
                  ? `${(Number(balance.data) / 10 ** 6).toFixed(5).toString()} ${symbol.trim().slice(0, 6)}${symbol.length > 6 ? "..." : ""}`
                  : `0 ${symbol.trim().slice(0, 6)}${symbol.length > 6 ? "..." : ""}`
                : `0 ${symbol.trim().slice(0, 6)}${symbol.length > 6 ? "..." : ""}`}
          </span>
        )}
        {option == "Sell" && (
          <span>{ isConnected ? `${(Number(balance?.data ?? 0) / 10 ** 6).toFixed(5).toString()} ${symbol.trim().slice(0, 6)}${symbol.length > 6 ? "..." : ""}` : `0 ${symbol.trim().slice(0, 6)}${symbol.length > 6 ? "..." : ""}`}</span>
        )}
      </div>
      <div className="relative">
        ...
        />
        <TokenLogo
          symbol={option == "Sell" ? symbol : pov == "Eth" ? "Eth" : symbol}
          coin={coin}
      ...
      <Quote
        ...
        option={option}
        setTrade={setTrade}
        setRequiredEth={setRequiredEth}
        balance={Number(ethBalance?.value ?? 0) / 10 ** 18}
        tokenBalance={Number(balance?.data ?? 0)}
        walletAddress={walletAddress}
      ></Quote>
      <div
        onClick={option === "Buy" ? _buyTokens : _sellTokens}
        className={cn(
          "flex justify-center items-center w-full h-10 rounded-xl text-neutral-800 font-bold border border-neutral-900",
          option == "Buy"
            ? trade.active
      ...
      >
        {buyLoading || processing || sellLoading || processing
          ? "Processing..."
...
const TokenLogo = memo(
  ({
    ...
    const displaySymbol =
      symbol === "Eth" ? "ETH" : (coin?.symbol || symbol).toUpperCase();
      ...
      return (
        <div className={cn("flex items-center gap-2", className)}>
          <span className="text-xs font-semibold text-neutral-200">
            {displaySymbol.length > 8
              ? `${displaySymbol.slice(0, 8)}…`
              : displaySymbol}
          </span>
          <img
    ...
    return (
      <span
        className={cn(
          "rounded-md bg-neutral-800 px-2 py-1 text-xs font-semibold text-neutral-200",
          className,
        )}
      >
...
const Quote = memo(
  ({
    ...
    pov: "Eth" | string;
    option: "Sell" | "Buy";
    setTrade: Dispatch<SetStateAction<tradeInterface>>;
    setRequiredEth: Dispatch<SetStateAction<number>>;
    balance: number;
    tokenBalance: number;
    walletAddress: `0x${string}`;
  }) => {
    const { data, isLoading, error } = useGetTokenReserves({
      key: coin.address as `0x${string}`,
    ...
    useEffect(() => {
      ...
      if (
        tokenReserveResult.status !== "success" ||
        vEthReserveResult.status !== "success"
      )
        return;
      ...
      const token_in_eth =
        ((tokenReserve / vEthReserve) * value * 10 ** 18) / 10 ** 6;
      const eth_in_token =
        ((vEthReserve / tokenReserve) * value * 10 ** 6) / 10 ** 18;
      ...
      setTrade((t) => ({
        ...t,
        active:
          option == "Buy" ? nextAllowTrade.allowBuy : nextAllowTrade.allowSell,
      }));
    ...
    return (
      <div className="w-full">
        {option == "Buy" && !allowTrade.allowBuy && (
          <div className="text-red-800">{allowTrade.buyMessage}</div>
        )}
        {option == "Sell" && !allowTrade.allowSell && (
          <div className="text-red-800">{allowTrade.sellMessage}</div>
        )}
        {option == "Buy" && pov == "Eth" && (
          <div>
        ...
        {option == "Buy" && pov != "Eth" && (
          <div>You pay: {price.eth_in_token} Eth</div>
        )}
        {option == "Sell" && <div>You recieve: {price.eth_in_token} Eth</div>}
      </div>
    );
  },
);
